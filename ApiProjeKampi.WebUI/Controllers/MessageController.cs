...
namespace ApiProjeKampi.WebUI.Controllers
{
    public class MessageController : Controller
    {
        ...
        public async Task<IActionResult> MessageList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7014/api/Messages");
        ...
        public async Task<IActionResult> CreateMessage(CreateMessageDto createMessageDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7014/api/Messages", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7014/api/Messages?id=" + id);
            return RedirectToAction("MessageList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7014/api/Messages/GetMessage?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateMessage(UpdateMessageDto updateMessageDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7014/api/Messages/", stringContent);
            return RedirectToAction("MessageList");
        }

        [HttpGet]
        public async Task<IActionResult> AnswerMessageWithGrokAI(int id, string prompt)
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7014/api/Messages/GetMessage?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        https://console.groq.com/keys
            ...
            var requestData = new
            {
                ...
                messages = new[]
                {
                    ...
                    {
                        role = "system",
                        content = "Sen bir restoran için kullanıcıların göndermiş olduğu mesajları detaylı ve olabildiğince olumlu müşteri menuniyeti gözeten cevaplar veren bir yapay zeka aracısın. Amacımız kullanıcı tarafından gönderilen mesajlara en olumlu ve maantıklı cevapları sunabilmek."
                    },
                    new
                    {
                        role = "user",
            ...
            var response = await client2.PostAsJsonAsync("https://api.groq.com/openai/v1/chat/completions", requestData);
        ...
        public async Task<IActionResult> SendMessage(CreateMessageDto createMessageDto)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(createMessageDto);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client2.PostAsync("https://localhost:7014/api/Messages", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("MessageList");
            }
